...
namespace JiraGit.Plugin
{
	...
	/// </summary>
    [ComVisible(true), ProgId("JiraGit.Plugin.TortoiseGitPlugin"), Guid(GUID), ClassInterface(ClassInterfaceType.AutoDual)]
    public class TortoiseGitPlugin : IDisposable, IBugTraqProvider, IBugTraqProvider2
	{
        //const string GUID = "CF732FD7-AA8A-4E9D-9E15-025E4D1A7E9D";
		const string GUID = "DF843FE8-BB9B-3D8C-8D04-136F5E2B8F0E";
        ...
        private string GetSetting(string name, string commonRoot) 
        {
            ...
            { //Read from Git...
                GitProperties props = new GitProperties(commonRoot);
			...
			get
			{
				if (_connector == null)
				{
					...
					if (string.IsNullOrEmpty(fullClass))
						throw new ApplicationException("Unable to locate JiraGit.Common.Interfaces.IIssuesService entry in app.config");
		...
		string GetParamDesc()
		{
			string message = "Parameter must be a valid absolute uri optionally with \r\n" +
					"a user name and password.  Leave blank to read value from Git \r\n" +
					"property named: '{0}'\r\n" +
